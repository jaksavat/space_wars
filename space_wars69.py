...
import glob
from river import stream, cluster, preprocessing, tree
import numpy as np
import pandas as pd
...
#Import Data Source
#df_each = [pd.read_csv(filename, header=None, delimiter=',', names=[f"A{i}" for i in range(11)])
#		  for filename in glob.glob("*.txt"), encoding= 'unicode_escape']

df = pd.read_csv("all_train_data.txt",header=None)
...
#Cluster Model : CluStream
clustream = cluster.CluStream(time_window=1,
                              n_macro_clusters=4,
                               seed=0,
                               halflife=0.4)

#Learning Cluster Model
...
#Label no.cluster to DataFrame
df['class'] = classes

mean_collected_coin = list(df.groupby(['class']).mean()[2])
mean_enemies_kills = list(df.groupby(['class']).mean()[3])
...
#import new datastreaming at 7 seconds
#df_datastreaming = pd.read_csv('n_train_data.txt',header=None)
#df_new = pd.concat([df_new,df_datastreaming],axis=0)

#Classification Model : HoeffdingTreeClassifier
...
#model = joblib.load('./model/model69.h5')
#N_LABELS = {1: 'Hardcore Achiever', 2: 'Hardcore Killer', 0: 'Casual Achiever', 3: 'Casual Killer'}
lst_type = []
#start_time2=time.time()

###########################################################

def prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, 
						  respawn_enemy_count, respawn_coin_count):
	global A0, A1
...
def prediction_user_type_dstream(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global B0, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10
	b0 = statistics.mean(B0[((len(B0) - 7)  if len(B0) >= 7 else 0):]) if len(B0) else 0
	b1 = statistics.mean(B1[((len(B1) - 7)  if len(B1) >= 7 else 0):]) if len(B1) else 0
	b2 = coin_count
...
def show_score(score, level, name, coin_count, font_size = 16, x=10, y=10, game_over=False, user_type=None, user_type_dstreaming=None):
	score_font = pygame.font.Font('freesansbold.ttf', font_size)
	...
	if user_type_dstreaming:
		user_type_dstreaming_text = score_font.render("OnlineML-You are : " + str(user_type_dstreaming), True, (255, 255, 0))
	...
	if user_type_dstreaming:
		y_pos = y_pos + 5 + font_size
		screen.blit(user_type_dstreaming_text, (x, y_pos))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type):
	...
	#message_display_center('(N) You\'re a ' + str(lst_type), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 4/10))
	message_display_center('OnlineML-You\'re a ' + max(set(lst_type), key = lst_type.count), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 3/10))
	message_display_center('You\'re a ' + user_type, font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 4/10))
...
PLAYER_NAME	= 'Group69_BayBaye&F'		#sws.PLAYER_NAME	# 'DAN'
...
def save_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	...
	A9 = respawn_enemy_count
	A10 = respawn_coin_count
	#A11 = round((start_time2-start_time1).total_seconds(),3)
	#A12 = round(A2/A11,3)
	#A13 = round(A4/A11,3)
...
#############################################################################
def save_dstreaming_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global B0, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10
	...
	if sum([b0, b1, b2, b3, b4, b5]) != 0:
		with open("n_train_data.txt", "a") as file_object:
...
start_timestreaming = datetime.now()
...
#			score += level_score_increase
		...
		if game_over:
			...
			#if ((datetime.now() - start_timestreaming).seconds) % timedelta( seconds = 7) == 0 :
			if ((datetime.now() - start_timestreaming).seconds) >= 7  :

				start_timestreaming = datetime.now()
				user_type_dstreaming = prediction_user_type_dstream(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
				save_dstreaming_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)

				if user_type_dstreaming:
					lst_type.append(str(user_type_dstreaming))
			...
			user_type = prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
			show_score(score, level, PLAYER_NAME, coin_count, user_type_dstreaming=user_type_dstreaming,user_type=user_type )
			#start_time2 = datetime.now()
			#time_d = start_time2-start_time1
			show_online_score()
